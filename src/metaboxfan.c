...
{    
    ...
    { .matches = { DMI_MATCH(DMI_BOARD_NAME, "L14xMU"), }, },
    {}
};
...
static uint8_t get_fan_count(void)
{
    if( dmi_match(DMI_BOARD_NAME, "W35_37ET") ||          //mainboards with 1 fan
        dmi_match(DMI_BOARD_NAME, "W350SS"  ) || dmi_match(DMI_BOARD_NAME, "L14xMU")  )
        return 1;
...
static int __init metaboxfan_init(void)
{
    acpi_handle ec_handle;
    
    if(strncmp(dmi_get_system_info(DMI_BOARD_VENDOR), "IT Channel Pty Ltd", 18) != 0)
        return -ENODEV;
...
static void __exit metaboxfan_exit(void)
{
    uint8_t i;
    for(i=0; i<fan_count;i++) {
        if(fan_auto[i] == 0) {
...
module_init(metaboxfan_init);
module_exit(metaboxfan_exit);
